...
void Protocol_Init(void)
{
    ...
    IPC6bits.U1IP = 1; //sets transmit interrupt priority to 1
    U1STAbits.UTXISEL1 = 0; //transmit interrupt happens when all characters have been transmitted
    U1STAbits.UTXISEL0 = 1; //have to kick start interrupts after every message 
...
void Protocol_SendMessage(unsigned char len, unsigned char ID, void *Payload)
{
    ...
    CircBuff_Add_Data('\n', &tx);
    IFS0bits.U1TXIF = 1; //kick starts transmit interrupt
    return;
}
...
{
    if (IFS0bits.U1TXIF) { //transmit interrupt only if transmit buffer not empty and transmit register not full
        if (!CircBuff_Test_Empty(tx) && !U1STAbits.UTXBF) {
            PutChar(CircBuff_Read_Data(&tx)); //puts next char into uart transmit buffer
        }
        IFS0bits.U1TXIF = 0; //clears transmit interrupt
    }
    if (IFS0bits.U1RXIF) { //receive interrupt
        if (U1STAbits.URXDA) { //if there is data in the receive buffer
            ...
            if (rxreg == '\n'){ //if last char of a message is detected, enter state machine to return message
                ...
                CircBuff_Init(&rx); //resets rx buffer after message read
            }
            while (U1STAbits.OERR && U1STAbits.URXDA) { //in case of overflow, read all data and clear overflow flag
                rxreg = U1RXREG; 
                CircBuff_Add_Data(rxreg, &rx);
            }
            U1STAbits.OERR = 0;
