...
{
    if (CircBuff_Test_Empty((*cb)) == 1) { //does not read if buffer empty, inputs end of string character
        return ('\0');
    }
...
int main(void)
{
    ...
    char string2[] = "nsidcpvjqroidnzdjdgdjntfuqglnmsehzmidfbrextncvcckgfcldsjpardxvnzkirfenngjztuzzzyizezahqaavkrushmswnpplnmdudhiunovfaksjzigxvpyanvj";
    char string3[MAX_BUFFER_LENGTH];
    //testing 129 character string
    for (i = 0; i < strlen(string2); i++) { //tests inputting a long string into buffer, throws out last char
